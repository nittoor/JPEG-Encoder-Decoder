...
char * ImageData::giveUpdatedCharStringForImage()
	{
		return displayImageString.data();
	}


bool ImageData::UpdateCharString(const int& x, const int& y,const unsigned char& R,const unsigned char& G,const unsigned char& B)
{
	int pointer= y*imgWidth+x;
	displayImageString[3*pointer]  =B;
	displayImageString[3*pointer+1]=G;
	displayImageString[3*pointer+2]=R;
